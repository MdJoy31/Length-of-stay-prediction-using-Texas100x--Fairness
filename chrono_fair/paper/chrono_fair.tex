...
\title{\Large \bfseries CHRONO-Fair: \\
  Time-Resolved, Anytime-Valid, Decomposed Monitoring \\
  of Counterfactual Verdict Flips in Clinical Machine Learning}
...
Let $f$ be a deployed binary classifier, $A$ a protected attribute, $X$ the remaining covariates, and $\mathrm{do}(A := a')$ a counterfactual intervention to set the protected attribute to $a'$ \cite{kusner2017}. For each patient $i$ arriving at index $t_i$, define the flip event time
\begin{equation}
...
The scalar VFR \cite{joy2025vfr} corresponds to $1 - S_a(\tau^*)$ where $S_a(t) = \exp(-\int_0^t \lambda_a(s) ds)$ is the no-flip survival; we recover VFR as the $\tau^* \to 0$ instantaneous case and as the $\tau^* \to \infty$ marginal probability of any flip. The Restricted Mean Flip Time at horizon $\tau^*$
\begin{equation}
...
\paragraph{Intersectional FDR.} For $m$ disjoint cells, we run $m$ independent monitors and apply online Benjamini-Hochberg at level $q$ to the implied p-values $p_t^{(c)} = 1/E_t^{(c)}$ at every observation tick. This provides simultaneous control of the false-discovery proportion across cells without sacrificing per-cell anytime validity.
...
When the intersectional monitor flags a cell, the Inspector Agent emits a structured report containing: cell label and size, anytime-valid e-value, flip-hazard ratio versus baseline, aleatoric/epistemic shares, dominant cause, recommended action, and the mapping to FDA PCCP \cite{fda2024pccp} clauses and EU AI Act \cite{eu2024aiact} articles plus the relevant STANDING Together \cite{ricci2025standing} recommendation. The reference implementation uses a deterministic templated narrator with no external LLM dependency; an LLM-narrator adapter (Anthropic Claude in the bundled implementation) can be plugged in for prose generation.
...
\caption{Detection delay (patients between drift onset and first alarm) across $50$ Monte-Carlo runs. Drift onset at $t = 5000$. CHRONO-Fair detects in $50/50$ runs at a mean delay of $856 \pm 228$ patients. ADWIN detects in $21/50$ runs at $3927 \pm 712$. A periodic batch test detects in $24/50$ runs but cannot control false alarms across peeks. Static VFR never alarms.}
\label{fig:delay}
\end{figure}
...
\caption{Controlled validation: $30$ trials per scenario. The decomposition labels every controlled trial correctly ($100\%$ accuracy). Right panel: empirical epistemic share by ground-truth scenario.}
\label{fig:decomp}
\end{figure}
