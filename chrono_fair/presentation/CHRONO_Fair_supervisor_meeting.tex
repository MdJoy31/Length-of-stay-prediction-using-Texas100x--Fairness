...
% Custom title chip (uses colorbox; no TikZ -> not fragile inside frame titles)
\newcommand{\chip}[2][amber]{%
  \begingroup\setlength{\fboxsep}{4pt}%
  \colorbox{#1}{\color{ink}\bfseries\small #2}\endgroup}
...
\vspace{-4pt}
\begin{teabox}[colframe=emerald]
\textbf{\color{emerald}\faIcon{lightbulb}\enspace Five things are new}, even though each statistical instrument is published elsewhere.
\end{teabox}
\vspace{-2pt}
\footnotesize
\begin{enumerate}\setlength\itemsep{1pt}
\item \textbf{New monitored object.} The \emph{per-cell counterfactual fairness-verdict flip} as a time-indexed stream. Prior anytime-valid work~[19, 20, 29] monitors aggregate risk. Prior fairness work~[13, 14, 15, 22] reports static metrics. Verdict flips per cell over time is not a monitored quantity in either literature.
\item \textbf{Coordinated framework.} Flip Hazard ($\rightarrow$ \emph{when}); per-cell e-process ($\rightarrow$ \emph{alarm}); aleatoric/epistemic decomposition ($\rightarrow$ \emph{triage}); RCAP ($\rightarrow$ \emph{regression}). All four feed one indicator and one Inspector report.
\item \textbf{Shrunk-GROW bet $\lambda_s$ + warm-up.} Finite-sample stabilisation of the GROW rule of~[29] for the small-flip-rate regime typical of clinical fairness.
\item \textbf{RCAP statistic.} $W_1$ is textbook~[23]; applying it to counterfactual rank-position distributions of an LOS-regression output is, to our knowledge, new.
\item \textbf{Anchor + controlled-replay evidence.} Real Texas-100X verdict instability at scale; then characterised under known drift.
\end{enumerate}
\vspace{-2pt}
{\scriptsize\color{muted}\itshape Honest attribution: hazard form~[24]; RMST; e-process wealth~[29]; Ville's inequality~[18]; aleatoric/epistemic split~[21, 30]; Wasserstein-1~[23]; step-wise BH~[26]. The contribution is the coordinated application, not the invention of new estimators.}
\end{frame}
